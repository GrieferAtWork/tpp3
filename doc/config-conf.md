...
L/C information on the associated [`TPP_TOK_SPACE`](../src/tpp-amalgamation.h#L11526) will fail.
...
- `2`: Do the inverse of flag `1` and pop a dummy-file off the `#include`-stack. Like the
       ...
       ignored.
- `3`: Set [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L16173) for the current text-file. When this flag is not
       supplied, [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L16173) is instead cleared for the current text-file.
       This flag requires [`TPP_HAVE_FILE_SYSHDR`](config-core.md#tpp_have_file_syshdr) to be enabled, otherwise it is ignored.
- `4`: Same as flag `3`, except for the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L16176) flag. Similarly, this
...
A keyword is considered to be an "identifier" if [`TPP_TOK_ISBUILTINKEYWORD()`](../src/tpp-amalgamation.h#L12959)
...
affect the behavior of [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L20840)
...
Configures if line-feed tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L21037)
...
Configures if whitespace tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L21037)
...
Configures if comment tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L21037)
...
Due to limitations related to when/how the contents of a [`tpp_file`](../src/tpp-amalgamation.h#L16268) can
be unloaded, said preceding whitespace will be considered part of the
[`TPP_TOK_SOL_SHELL_COMMENT`](../src/tpp-amalgamation.h#L11709) token)
...
resulting [`TPP_TOK_SOL_SLASH_COMMENT`](../src/tpp-amalgamation.h#L11715) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
appears as the first character of the relevant line, or is preceded by
nothing but whitespace (any preceding whitespace will be part of the
resulting [`TPP_TOK_SOL_AT_COMMENT`](../src/tpp-amalgamation.h#L11721) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
hex nibbles to be specified. A warning [`TPP_W_CHARACTER_TOO_LARGE`](../src/tpp-amalgamation.h#L4029) is emitted if the hex-sequence is too
large to fit into [`tpp_uintmax`](../src/tpp-amalgamation.h#L4496), or the output string format.
...
This affects the behavior of [`tpp_lexer_parsestring_ex()`](../src/tpp-amalgamation.h#L21650) and its
companion [`tpp_lexer_parsestring_cb()`](../src/tpp-amalgamation.h#L21690), such that they will only yield
...
automatically enabled under [`TPP_PROFILE_DEFAULT`](../src/tpp-amalgamation.h#L5032), so if you
...
When [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L16173) is set during `#include` (i.e. *NOT* via `#pragma GCC system_header`),
then the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L16176) flag should be set alongside [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L16173).
...
Enable support for `0x` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L21474) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L11584) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
...
Enable support for `0b` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L21474) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L11584) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
...
Enable support for `0o` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L21474) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L11584) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
