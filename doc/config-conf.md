...
L/C information on the associated [`TPP_TOK_SPACE`](../src/tpp-amalgamation.h#L14714) will fail.
...
- `2`: Do the inverse of flag `1` and pop a dummy-file off the `#include`-stack. Like the
       ...
       ignored.
- `3`: Set [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L19466) for the current text-file. When this flag is not
       supplied, [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L19466) is instead cleared for the current text-file.
       This flag requires [`TPP_HAVE_FILE_SYSHDR`](config-core.md#tpp_have_file_syshdr) to be enabled, otherwise it is ignored.
- `4`: Same as flag `3`, except for the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L19469) flag. Similarly, this
...
[`tpp_lexer_isidentifier()`](../src/tpp-amalgamation.h#L25816) (see [`TPP_HAVE_LEXER_ISIDENTIFIER`](config-core.md#tpp_have_lexer_isidentifier))
...
affect the behavior of [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L25079)
...
Configures if line-feed tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L25276)
...
Configures if whitespace tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L25276)
...
Configures if comment tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L25276)
...
Due to limitations related to when/how the contents of a [`tpp_file`](../src/tpp-amalgamation.h#L19561) can
be unloaded, said preceding whitespace will be considered part of the
[`TPP_TOK_SOL_SHELL_COMMENT`](../src/tpp-amalgamation.h#L14897) token)
...
resulting [`TPP_TOK_SOL_SLASH_COMMENT`](../src/tpp-amalgamation.h#L14903) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
appears as the first character of the relevant line, or is preceded by
nothing but whitespace (any preceding whitespace will be part of the
resulting [`TPP_TOK_SOL_AT_COMMENT`](../src/tpp-amalgamation.h#L14909) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
large to fit into [`tpp_uintmax`](../src/tpp-amalgamation.h#L5488), or the output string format.
...
This affects the behavior of [`tpp_lexer_parsestring_ex()`](../src/tpp-amalgamation.h#L26023) and its
companion [`tpp_lexer_parsestring_cb()`](../src/tpp-amalgamation.h#L26063), such that they will only yield
...
automatically enabled under [`TPP_PROFILE_DEFAULT`](../src/tpp-amalgamation.h#L6030), so if you
...
When [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L19466) is set during `#include` (i.e. *NOT* via `#pragma GCC system_header`),
then the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L19469) flag should be set alongside [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L19466).
...
When enabled, [`TPP_WSTATE_WARN`](../src/tpp-amalgamation.h#L21824) is treated as [`TPP_WSTATE_ERROR_OR_FATAL`](../src/tpp-amalgamation.h#L21827) instead
...
```c
((TPP_PROFILE == TPP_PROFILE_ALL) && TPP_HAVE_WARNINGS) ? TPP_CONF_EXT0 : 0
```
...
When enabled, the [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L19466) flag of files is ignored,
...
```c
((TPP_PROFILE == TPP_PROFILE_ALL) && TPP_HAVE_FILE_SYSHDR) ? TPP_CONF_EXT0 : 0
```
...
Enable support for `0x` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L25847) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L14772) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
...
Enable support for `0b` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L25847) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L14772) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
...
Enable support for `0o` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L25847) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L14772) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
