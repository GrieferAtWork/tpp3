...
L/C information on the associated [`TPP_TOK_SPACE`](../src/tpp-amalgamation.h#L16482) will fail.
...
- `2`: Do the inverse of flag `1` and pop a dummy-file off the `#include`-stack. Like the
       ...
       ignored.
- `3`: Set [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L21413) for the current text-file. When this flag is not
       supplied, [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L21413) is instead cleared for the current text-file.
       This flag requires [`TPP_HAVE_FILE_SYSHDR`](config-core.md#tpp_have_file_syshdr) to be enabled, otherwise it is ignored.
- `4`: Same as flag `3`, except for the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L21422) flag. Similarly, this
...
[`tpp_lexer_isidentifier()`](../src/tpp-amalgamation.h#L29725) (see [`TPP_HAVE_LEXER_ISIDENTIFIER`](config-core.md#tpp_have_lexer_isidentifier))
...
affect the behavior of [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L28949)
...
Configures if line-feed tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L29146)
...
Configures if whitespace tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L29146)
...
Configures if comment tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L29146)
...
Due to limitations related to when/how the contents of a [`tpp_file`](../src/tpp-amalgamation.h#L21542) can
be unloaded, said preceding whitespace will be considered part of the
[`TPP_TOK_SOL_SHELL_COMMENT`](../src/tpp-amalgamation.h#L16665) token)
...
resulting [`TPP_TOK_SOL_SLASH_COMMENT`](../src/tpp-amalgamation.h#L16671) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
appears as the first character of the relevant line, or is preceded by
nothing but whitespace (any preceding whitespace will be part of the
resulting [`TPP_TOK_SOL_AT_COMMENT`](../src/tpp-amalgamation.h#L16677) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
Whenenver [`tpp_lexer_openfile()`](../src/tpp-amalgamation.h#L27887) is called and this feature is enabled,
...
- [`tpp_lexer_openfile()`](../src/tpp-amalgamation.h#L27887) will open file `f"{headof(relative_to)}/foo/real/fum.h"`,
...
- `header.gcc` files are still parsed using [`tpp_file`](../src/tpp-amalgamation.h#L21542), meaning that
...
This affects the behavior of [`tpp_lexer_parsestring_ex()`](../src/tpp-amalgamation.h#L29958) and its
companion [`tpp_lexer_parsestring_cb()`](../src/tpp-amalgamation.h#L29998), such that they will only yield
...
expression handler is specified in [`tpp_lexer_decodestring_config`](../src/tpp-amalgamation.h#L29803),
and one ends up being needed (for one reason or another), use
[`tpp_lexer_callhook_parseexpr()`](../src/tpp-amalgamation.h#L28207) (s.a. [`TPP_HAVE_PARSEEXPR_HOOK`](config-hook.md#tpp_have_parseexpr_hook))
to parse an expression, and [`tpp_expr_value_printstr()`](../src/tpp-amalgamation.h#L15566) (s.a.
...
When [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L21413) is set during `#include` (i.e. *NOT* via `#pragma GCC system_header`),
then the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L21422) flag should be set alongside [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L21413).
...
When enabled, [`TPP_WSTATE_WARN`](../src/tpp-amalgamation.h#L24105) is treated as [`TPP_WSTATE_ERROR_OR_FATAL`](../src/tpp-amalgamation.h#L24108) instead
...
When enabled, the [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L21413) flag of files is ignored,
...
Enable support for `0x` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L29756) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L16540) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
...
Enable support for `0b` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L29756) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L16540) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
...
Enable support for `0o` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L29756) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L16540) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
