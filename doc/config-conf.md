...
L/C information on the associated [`TPP_TOK_SPACE`](../src/tpp-amalgamation.h#L15946) will fail.
...
- `2`: Do the inverse of flag `1` and pop a dummy-file off the `#include`-stack. Like the
       ...
       ignored.
- `3`: Set [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L20748) for the current text-file. When this flag is not
       supplied, [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L20748) is instead cleared for the current text-file.
       This flag requires [`TPP_HAVE_FILE_SYSHDR`](config-core.md#tpp_have_file_syshdr) to be enabled, otherwise it is ignored.
- `4`: Same as flag `3`, except for the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L20751) flag. Similarly, this
...
Due to limitations related to when/how the contents of a [`tpp_file`](../src/tpp-amalgamation.h#L20842) can
be unloaded, said preceding whitespace will be considered part of the
[`TPP_TOK_SOL_SHELL_COMMENT`](../src/tpp-amalgamation.h#L16129) token)
...
resulting [`TPP_TOK_SOL_SLASH_COMMENT`](../src/tpp-amalgamation.h#L16135) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
appears as the first character of the relevant line, or is preceded by
nothing but whitespace (any preceding whitespace will be part of the
resulting [`TPP_TOK_SOL_AT_COMMENT`](../src/tpp-amalgamation.h#L16141) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
- `header.gcc` files are still parsed using [`tpp_file`](../src/tpp-amalgamation.h#L20842), meaning that
...
When [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L20748) is set during `#include` (i.e. *NOT* via `#pragma GCC system_header`),
then the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L20751) flag should be set alongside [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L20748).
...
When enabled, [`TPP_WSTATE_WARN`](../src/tpp-amalgamation.h#L23390) is treated as [`TPP_WSTATE_ERROR_OR_FATAL`](../src/tpp-amalgamation.h#L23393) instead
...
When enabled, the [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L20748) flag of files is ignored,
