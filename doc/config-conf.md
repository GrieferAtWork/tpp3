...
L/C information on the associated [`TPP_TOK_SPACE`](../src/tpp-amalgamation.h#L16193) will fail.
...
- `2`: Do the inverse of flag `1` and pop a dummy-file off the `#include`-stack. Like the
       ...
       ignored.
- `3`: Set [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L21120) for the current text-file. When this flag is not
       supplied, [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L21120) is instead cleared for the current text-file.
       This flag requires [`TPP_HAVE_FILE_SYSHDR`](config-core.md#tpp_have_file_syshdr) to be enabled, otherwise it is ignored.
- `4`: Same as flag `3`, except for the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L21129) flag. Similarly, this
...
[`tpp_lexer_isidentifier()`](../src/tpp-amalgamation.h#L29377) (see [`TPP_HAVE_LEXER_ISIDENTIFIER`](config-core.md#tpp_have_lexer_isidentifier))
...
affect the behavior of [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L28638)
...
Configures if line-feed tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L28835)
...
Configures if whitespace tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L28835)
...
Configures if comment tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L28835)
...
Due to limitations related to when/how the contents of a [`tpp_file`](../src/tpp-amalgamation.h#L21249) can
be unloaded, said preceding whitespace will be considered part of the
[`TPP_TOK_SOL_SHELL_COMMENT`](../src/tpp-amalgamation.h#L16376) token)
...
resulting [`TPP_TOK_SOL_SLASH_COMMENT`](../src/tpp-amalgamation.h#L16382) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
appears as the first character of the relevant line, or is preceded by
nothing but whitespace (any preceding whitespace will be part of the
resulting [`TPP_TOK_SOL_AT_COMMENT`](../src/tpp-amalgamation.h#L16388) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
Whenenver [`tpp_lexer_openfile()`](../src/tpp-amalgamation.h#L27576) is called and this feature is enabled,
...
- [`tpp_lexer_openfile()`](../src/tpp-amalgamation.h#L27576) will open file `f"{headof(relative_to)}/foo/real/fum.h"`,
...
- `header.gcc` files are still parsed using [`tpp_file`](../src/tpp-amalgamation.h#L21249), meaning that
...
This affects the behavior of [`tpp_lexer_parsestring_ex()`](../src/tpp-amalgamation.h#L29610) and its
companion [`tpp_lexer_parsestring_cb()`](../src/tpp-amalgamation.h#L29650), such that they will only yield
...
Extension to [`TPP_HAVE_STRING_FORMAT`](../src/tpp-amalgamation.h#L9233): when no format-expression handler is specified
in [`tpp_lexer_decodestring_config`](../src/tpp-amalgamation.h#L29455), and one ends up being needed, rather than raising
a [`TPP_W_UNSUPPORTED_FORMAT_STRING_ESCAPE`](../src/tpp-amalgamation.h#L4589) warning (see [`TPP_HAVE_TPP_W_UNSUPPORTED_FORMAT_STRING_ESCAPE`](config-warn.md#tpp_have_tpp_w_unsupported_format_string_escape)),
...
When [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L21120) is set during `#include` (i.e. *NOT* via `#pragma GCC system_header`),
then the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L21129) flag should be set alongside [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L21120).
...
When enabled, [`TPP_WSTATE_WARN`](../src/tpp-amalgamation.h#L23808) is treated as [`TPP_WSTATE_ERROR_OR_FATAL`](../src/tpp-amalgamation.h#L23811) instead
...
When enabled, the [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L21120) flag of files is ignored,
...
Enable support for `0x` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L29408) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L16251) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
...
Enable support for `0b` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L29408) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L16251) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
...
Enable support for `0o` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L29408) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L16251) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
