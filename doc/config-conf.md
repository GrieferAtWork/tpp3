...
L/C information on the associated [`TPP_TOK_SPACE`](../src/tpp-amalgamation.h#L12448) will fail.
...
- `2`: Do the inverse of flag `1` and pop a dummy-file off the `#include`-stack. Like the
       ...
       ignored.
- `3`: Set [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L17128) for the current text-file. When this flag is not
       supplied, [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L17128) is instead cleared for the current text-file.
       This flag requires [`TPP_HAVE_FILE_SYSHDR`](config-core.md#tpp_have_file_syshdr) to be enabled, otherwise it is ignored.
- `4`: Same as flag `3`, except for the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L17131) flag. Similarly, this
...
A keyword is considered to be an "identifier" if [`TPP_TOK_ISBUILTINKEYWORD()`](../src/tpp-amalgamation.h#L13881)
...
affect the behavior of [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L21802)
...
Configures if line-feed tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L21999)
...
Configures if whitespace tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L21999)
...
Configures if comment tokens should be forwarded, or filtered by [`tpp_lexer_yieldpp()`](../src/tpp-amalgamation.h#L21999)
...
Due to limitations related to when/how the contents of a [`tpp_file`](../src/tpp-amalgamation.h#L17223) can
be unloaded, said preceding whitespace will be considered part of the
[`TPP_TOK_SOL_SHELL_COMMENT`](../src/tpp-amalgamation.h#L12631) token)
...
resulting [`TPP_TOK_SOL_SLASH_COMMENT`](../src/tpp-amalgamation.h#L12637) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
appears as the first character of the relevant line, or is preceded by
nothing but whitespace (any preceding whitespace will be part of the
resulting [`TPP_TOK_SOL_AT_COMMENT`](../src/tpp-amalgamation.h#L12643) token; see [`TPP_HAVE_TOK_SOL_SHELL_COMMENT`](#tpp_have_tok_sol_shell_comment))
...
hex nibbles to be specified. A warning [`TPP_W_CHARACTER_TOO_LARGE`](../src/tpp-amalgamation.h#L4802) is emitted if the hex-sequence is too
large to fit into [`tpp_uintmax`](../src/tpp-amalgamation.h#L5270), or the output string format.
...
This affects the behavior of [`tpp_lexer_parsestring_ex()`](../src/tpp-amalgamation.h#L22636) and its
companion [`tpp_lexer_parsestring_cb()`](../src/tpp-amalgamation.h#L22676), such that they will only yield
...
automatically enabled under [`TPP_PROFILE_DEFAULT`](../src/tpp-amalgamation.h#L5806), so if you
...
When [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L17128) is set during `#include` (i.e. *NOT* via `#pragma GCC system_header`),
then the [`TPP_FILE_FLAGS_EXTERN_C`](../src/tpp-amalgamation.h#L17131) flag should be set alongside [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L17128).
...
When enabled, [`TPP_WSTATE_WARN`](../src/tpp-amalgamation.h#L19288) is treated as [`TPP_WSTATE_ERROR_OR_FATAL`](../src/tpp-amalgamation.h#L19291) instead
...
When enabled, the [`TPP_FILE_FLAGS_SYSHDR`](../src/tpp-amalgamation.h#L17128) flag of files is ignored,
...
Enable support for `0x` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L22460) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L12506) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
...
Enable support for `0b` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L22460) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L12506) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
...
Enable support for `0o` literals in [`tpp_lexer_decodeint()`](../src/tpp-amalgamation.h#L22460) when parsing [`TPP_TOK_C_INT`](../src/tpp-amalgamation.h#L12506) (see [`TPP_HAVE_TOK_C_INT`](#tpp_have_tok_c_int))
