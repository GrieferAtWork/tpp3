...
| `TPP_PROFILE_C` | Use this if you're writing a C compiler: pretty much the same as `TPP_PROFILE_DEFAULT`, but some features required for that would otherwise be runtime-configurable become hard-enabled (such as [`TPP_HAVE_BSE`](config-conf.md#tpp_have_bse)) |
