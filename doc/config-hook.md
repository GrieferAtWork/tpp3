...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L27132) to print warning messages.
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L27132) to handle warning notifications. Can be
...
about the just-pushed file can be retrieved by examining [`tpp_lexer_getfile(LEXER)`](../src/tpp-amalgamation.h#L24764).

Notes:

- This hook can be used by a frontend to implement stuff like GCC's `--trace-includes`.
- This hook is *NOT* called for [`tpp_file_subtext_push()`](../src/tpp-amalgamation.h#L20594) or [`tpp_file_pushdummy()`](../src/tpp-amalgamation.h#L20988)
...
by examining [`tpp_lexer_getfile(LEXER)`](../src/tpp-amalgamation.h#L24764).

Notes:

- When files are popped by [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L26144) within a
  [`tpp_lexer_manualpopfile_start()`](../src/tpp-amalgamation.h#L26142)-region, this hook is called during the
  *commit* phase (i.e.: by [`tpp_lexer_manualpopfile_break_commit()`](../src/tpp-amalgamation.h#L26149)),
  rather than [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L26144) as one might suspect at first.
- This hook is *NOT* called by [`tpp_file_subtext_pop()`](../src/tpp-amalgamation.h#L20617) or [`tpp_file_popdummy()`](../src/tpp-amalgamation.h#L21001)
...
been populated by [`tpp_lexer_yieldraw_at_include_string_blocking()`](../src/tpp-amalgamation.h#L26320) (and macros
were also already expanded), and the current token is [`TPP_TOK_INCPATH_DQUOTE`](../src/tpp-amalgamation.h#L26289) or
[`TPP_TOK_INCPATH_LANGLE`](../src/tpp-amalgamation.h#L26290).
...
To gain access to the `#include`-string, you must use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L26355)
...
the error (by returning something other than [`TPP_ENOENT`](../src/tpp-amalgamation.h#L13059)), or log the error to
...
to know what that string says, you can use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L26355) to decode it.
...
- This hook is *ONLY* invoked when `#define` is encountered, or
  ...
  definition.
- Calls to [`tpp_lexer_define()`](../src/tpp-amalgamation.h#L25787) or other related functions will
...
no predefined definition: [`_TPP_KEYWORD_MACRO_UNDEFINED`](../src/tpp-amalgamation.h#L21348)). This
hook is imply called as part of the process of evaluating `#undef`

This hook is *ONLY* invoked when `#undef` is encountered.
Calls to [`tpp_lexer_undef()`](../src/tpp-amalgamation.h#L25799), [`tpp_keyword_undef()`](../src/tpp-amalgamation.h#L21807), or other
...
Extra callback invoked by [`tpp_lexer_foreach_include_path()`](../src/tpp-amalgamation.h#L26386) at different
...
Extra callback invoked by [`tpp_lexer_foreach_embed_path()`](../src/tpp-amalgamation.h#L26428) at different points
...
Called by [`tpp_lexer_decodestring()`](../src/tpp-amalgamation.h#L26889) when an unknown `\`-escape sequence is encountered.
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L27132) just before it's about to return [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L13107)
This hook can be used to do additional state changes that may be necessary by the
hosting application in order to handle the resulting [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L13107)
...
Called by [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L25984) when [`TPP_HAVE_SMART_FLOAT_TOKENS`](config-conf.md#tpp_have_smart_float_tokens) is enabled and
...
yet, though can be loaded using [`tpp_lexer_readchar()`](../src/tpp-amalgamation.h#L25888) and [`tpp_lexer_readunichar()`](../src/tpp-amalgamation.h#L25897))
