...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L20769) to print warning messages
Potentially unused if [`TPP_HAVE_WARNHANDLER_HOOK`](../src/tpp-amalgamation.h#L7937) is also overwritten
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L20769) to handle warning notifications. Can be
...
Extra callback invoked by [`tpp_lexer_foreach_include_path()`](../src/tpp-amalgamation.h#L20088) at diffrent
...
Called by [`tpp_lexer_decodestring()`](../src/tpp-amalgamation.h#L20513) when an unknown `\`-escape sequence is encountered
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L20769) just before it's about to return [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L9618)
This hook can be used to do additional state changes that may be necessary by the
hosting application in order to handle the resulting [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L9618)
...
Called by [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L19742) when [`TPP_HAVE_SMART_FLOAT_TOKENS`](config-conf.md#tpp_have_smart_float_tokens) is enabled and
...
loaded yet, though can be loaded using [`tpp_lexer_readchar()`](../src/tpp-amalgamation.h#L19646))
