...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L30090) to print warning messages.
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L30090) to handle warning notifications. Can be
...
about the just-pushed file can be retrieved by examining [`tpp_lexer_getfile(LEXER)`](../src/tpp-amalgamation.h#L27464).

Notes:

- This hook can be used by a frontend to implement stuff like GCC's `--trace-includes`.
- This hook is *NOT* called for [`tpp_file_subtext_push()`](../src/tpp-amalgamation.h#L22107) or [`tpp_file_pushdummy()`](../src/tpp-amalgamation.h#L22532)
...
by examining [`tpp_lexer_getfile(LEXER)`](../src/tpp-amalgamation.h#L27464).

Notes:

- When files are popped by [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L29076) within a
  [`tpp_lexer_manualpopfile_start()`](../src/tpp-amalgamation.h#L29074)-region, this hook is called during the
  *commit* phase (i.e.: by [`tpp_lexer_manualpopfile_break_commit()`](../src/tpp-amalgamation.h#L29081)),
  rather than [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L29076) as one might suspect at first.
- This hook is *NOT* called by [`tpp_file_subtext_pop()`](../src/tpp-amalgamation.h#L22133) or [`tpp_file_popdummy()`](../src/tpp-amalgamation.h#L22546)
...
been populated by [`tpp_lexer_yieldraw_at_include_string_blocking()`](../src/tpp-amalgamation.h#L29252) (and macros
were also already expanded), and the current token is [`TPP_TOK_INCPATH_DQUOTE`](../src/tpp-amalgamation.h#L29221) or
[`TPP_TOK_INCPATH_LANGLE`](../src/tpp-amalgamation.h#L29222).
...
To gain access to the `#include`-string, you must use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L29287)
...
to know what that string says, you can use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L29287) to decode it.
...
- This hook is *ONLY* invoked when `#define` is encountered, or
  ...
  definition.
- Calls to [`tpp_lexer_define()`](../src/tpp-amalgamation.h#L28720) or other related functions will
...
no predefined definition: [`_TPP_KEYWORD_MACRO_UNDEFINED`](../src/tpp-amalgamation.h#L22895)). This
hook is imply called as part of the process of evaluating `#undef`

This hook is *ONLY* invoked when `#undef` is encountered.
Calls to [`tpp_lexer_undef()`](../src/tpp-amalgamation.h#L28732), [`tpp_keyword_undef()`](../src/tpp-amalgamation.h#L23463), or other
...
Extra callback invoked by [`tpp_lexer_foreach_include_path()`](../src/tpp-amalgamation.h#L29318) at different
...
Extra callback invoked by [`tpp_lexer_foreach_embed_path()`](../src/tpp-amalgamation.h#L29360) at different points
...
Called by [`tpp_lexer_decodestring()`](../src/tpp-amalgamation.h#L29849) when an unknown `\`-escape sequence is encountered.
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L30090) just before it's about to return [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L13909)
...
Called by [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L28916) when [`TPP_HAVE_SMART_FLOAT_TOKENS`](config-conf.md#tpp_have_smart_float_tokens) is enabled and
...
yet, though can be loaded using [`tpp_lexer_readchar()`](../src/tpp-amalgamation.h#L28821) and [`tpp_lexer_readunichar()`](../src/tpp-amalgamation.h#L28834))
