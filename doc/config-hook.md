...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L26683) to print warning messages.
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L26683) to handle warning notifications. Can be
...
about the just-pushed file can be retrieved by examining [`tpp_lexer_getfile(LEXER)`](../src/tpp-amalgamation.h#L24338).

Notes:

- This hook can be used by a frontend to implement stuff like GCC's `--trace-includes`.
- This hook is *NOT* called for [`tpp_file_subtext_push()`](../src/tpp-amalgamation.h#L20189) or [`tpp_file_pushdummy()`](../src/tpp-amalgamation.h#L20581)
...
by examining [`tpp_lexer_getfile(LEXER)`](../src/tpp-amalgamation.h#L24338).

Notes:

- When files are popped by [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L25701) within a
  [`tpp_lexer_manualpopfile_start()`](../src/tpp-amalgamation.h#L25699)-region, this hook is called during the
  *commit* phase (i.e.: by [`tpp_lexer_manualpopfile_break_commit()`](../src/tpp-amalgamation.h#L25706)),
  rather than [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L25701) as one might suspect at first.
- This hook is *NOT* called by [`tpp_file_subtext_pop()`](../src/tpp-amalgamation.h#L20212) or [`tpp_file_popdummy()`](../src/tpp-amalgamation.h#L20594)
...
been populated by [`tpp_lexer_yieldraw_at_include_string_blocking()`](../src/tpp-amalgamation.h#L25877) (and macros
were also already expanded), and the current token is [`TPP_TOK_INCPATH_DQUOTE`](../src/tpp-amalgamation.h#L25846) or
[`TPP_TOK_INCPATH_LANGLE`](../src/tpp-amalgamation.h#L25847).
...
To gain access to the `#include`-string, you must use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L25912)
...
the error (by returning something other than [`TPP_ENOENT`](../src/tpp-amalgamation.h#L12755)), or log the error to
...
to know what that string says, you can use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L25912) to decode it.
...
- This hook is *ONLY* invoked when `#define` is encountered, or
  ...
  definition.
- Calls to [`tpp_lexer_define()`](../src/tpp-amalgamation.h#L25350) or other related functions will
...
no predefined definition: [`_TPP_KEYWORD_MACRO_UNDEFINED`](../src/tpp-amalgamation.h#L20939)). This
hook is imply called as part of the process of evaluating `#undef`

This hook is *ONLY* invoked when `#undef` is encountered.
Calls to [`tpp_lexer_undef()`](../src/tpp-amalgamation.h#L25358), [`tpp_keyword_undef()`](../src/tpp-amalgamation.h#L21387), or other
...
Extra callback invoked by [`tpp_lexer_foreach_include_path()`](../src/tpp-amalgamation.h#L25943) at different
...
Extra callback invoked by [`tpp_lexer_foreach_embed_path()`](../src/tpp-amalgamation.h#L25985) at different points
...
Called by [`tpp_lexer_decodestring()`](../src/tpp-amalgamation.h#L26446) when an unknown `\`-escape sequence is encountered.
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L26683) just before it's about to return [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L12803)
This hook can be used to do additional state changes that may be necessary by the
hosting application in order to handle the resulting [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L12803)
...
Called by [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L25541) when [`TPP_HAVE_SMART_FLOAT_TOKENS`](config-conf.md#tpp_have_smart_float_tokens) is enabled and
...
yet, though can be loaded using [`tpp_lexer_readchar()`](../src/tpp-amalgamation.h#L25445) and [`tpp_lexer_readunichar()`](../src/tpp-amalgamation.h#L25454))
