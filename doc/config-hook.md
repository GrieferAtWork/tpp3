...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L30100) to print warning messages.
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L30100) to handle warning notifications. Can be
...
about the just-pushed file can be retrieved by examining [`tpp_lexer_getfile(LEXER)`](../src/tpp-amalgamation.h#L27474).

Notes:

- This hook can be used by a frontend to implement stuff like GCC's `--trace-includes`.
- This hook is *NOT* called for [`tpp_file_subtext_push()`](../src/tpp-amalgamation.h#L22111) or [`tpp_file_pushdummy()`](../src/tpp-amalgamation.h#L22536)
...
by examining [`tpp_lexer_getfile(LEXER)`](../src/tpp-amalgamation.h#L27474).

Notes:

- When files are popped by [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L29086) within a
  [`tpp_lexer_manualpopfile_start()`](../src/tpp-amalgamation.h#L29084)-region, this hook is called during the
  *commit* phase (i.e.: by [`tpp_lexer_manualpopfile_break_commit()`](../src/tpp-amalgamation.h#L29091)),
  rather than [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L29086) as one might suspect at first.
- This hook is *NOT* called by [`tpp_file_subtext_pop()`](../src/tpp-amalgamation.h#L22137) or [`tpp_file_popdummy()`](../src/tpp-amalgamation.h#L22550)
...
been populated by [`tpp_lexer_yieldraw_at_include_string_blocking()`](../src/tpp-amalgamation.h#L29262) (and macros
were also already expanded), and the current token is [`TPP_TOK_INCPATH_DQUOTE`](../src/tpp-amalgamation.h#L29231) or
[`TPP_TOK_INCPATH_LANGLE`](../src/tpp-amalgamation.h#L29232).
...
To gain access to the `#include`-string, you must use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L29297)
...
to know what that string says, you can use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L29297) to decode it.
...
- This hook is *ONLY* invoked when `#define` is encountered, or
  ...
  definition.
- Calls to [`tpp_lexer_define()`](../src/tpp-amalgamation.h#L28730) or other related functions will
...
no predefined definition: [`_TPP_KEYWORD_MACRO_UNDEFINED`](../src/tpp-amalgamation.h#L22899)). This
hook is imply called as part of the process of evaluating `#undef`

This hook is *ONLY* invoked when `#undef` is encountered.
Calls to [`tpp_lexer_undef()`](../src/tpp-amalgamation.h#L28742), [`tpp_keyword_undef()`](../src/tpp-amalgamation.h#L23467), or other
...
Extra callback invoked by [`tpp_lexer_foreach_include_path()`](../src/tpp-amalgamation.h#L29328) at different
...
Extra callback invoked by [`tpp_lexer_foreach_embed_path()`](../src/tpp-amalgamation.h#L29370) at different points
...
Called by [`tpp_lexer_decodestring()`](../src/tpp-amalgamation.h#L29859) when an unknown `\`-escape sequence is encountered.
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L30100) just before it's about to return [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L13909)
...
Called by [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L28926) when [`TPP_HAVE_SMART_FLOAT_TOKENS`](config-conf.md#tpp_have_smart_float_tokens) is enabled and
...
yet, though can be loaded using [`tpp_lexer_readchar()`](../src/tpp-amalgamation.h#L28831) and [`tpp_lexer_readunichar()`](../src/tpp-amalgamation.h#L28844))
