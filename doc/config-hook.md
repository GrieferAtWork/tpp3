...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L25836) to print warning messages
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L25836) to handle warning notifications. Can be
...
Information about the just-pushed file can be retrieved by examining [`tpp_lexer_getfile(self)`](../src/tpp-amalgamation.h#L23596)
This hook can be used by a frontend to implement stuff like GCC's `--trace-includes`.
WARNING: *NOT* Called for [`tpp_file_subtext_push()`](../src/tpp-amalgamation.h#L19868) or [`tpp_file_pushdummy()`](../src/tpp-amalgamation.h#L20245)
...
by examining [`tpp_lexer_getfile(self)`](../src/tpp-amalgamation.h#L23596). Note that this hook is called
during the file-pop *commit* phase ([`tpp_lexer_manualpopfile_break_commit()`](../src/tpp-amalgamation.h#L24850))
but is *NOT* called by [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L24845).
WARNING: *NOT* Called for [`tpp_file_subtext_pop()`](../src/tpp-amalgamation.h#L19891) or [`tpp_file_popdummy()`](../src/tpp-amalgamation.h#L20258)
...
been populated by [`tpp_lexer_yieldraw_at_include_string_blocking()`](../src/tpp-amalgamation.h#L25021) (and macros
were also already expanded), and the current token is [`TPP_TOK_INCPATH_DQUOTE`](../src/tpp-amalgamation.h#L24990) or
[`TPP_TOK_INCPATH_LANGLE`](../src/tpp-amalgamation.h#L24991).
...
To gain access to the `#include`-string, you must use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L25056)
...
something other than [`TPP_ENOENT`](../src/tpp-amalgamation.h#L12523)), or log the error to implement something like GCC's `-MG`
commandline switch.
...
to know what that string says, you can use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L25056) to decode it.
...
Calls to [`tpp_lexer_define()`](../src/tpp-amalgamation.h#L24494) or other related functions will
...
no predefined definition: [`_TPP_KEYWORD_MACRO_UNDEFINED`](../src/tpp-amalgamation.h#L20595)). This
hook is imply called as part of the process of evaluating `#undef`

This hook is *ONLY* invoked when `#undef` is encountered.
Calls to [`tpp_lexer_undef()`](../src/tpp-amalgamation.h#L24502), [`tpp_keyword_undef()`](../src/tpp-amalgamation.h#L21038), or other
...
Extra callback invoked by [`tpp_lexer_foreach_include_path()`](../src/tpp-amalgamation.h#L25087) at different
...
Extra callback invoked by [`tpp_lexer_foreach_embed_path()`](../src/tpp-amalgamation.h#L25129) at different points
...
Called by [`tpp_lexer_decodestring()`](../src/tpp-amalgamation.h#L25592) when an unknown `\`-escape sequence is encountered
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L25836) just before it's about to return [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L12571)
This hook can be used to do additional state changes that may be necessary by the
hosting application in order to handle the resulting [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L12571)
...
Called by [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L24685) when [`TPP_HAVE_SMART_FLOAT_TOKENS`](config-conf.md#tpp_have_smart_float_tokens) is enabled and
...
loaded yet, though can be loaded using [`tpp_lexer_readchar()`](../src/tpp-amalgamation.h#L24589))
