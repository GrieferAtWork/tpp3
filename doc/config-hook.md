...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L29812) to print warning messages.
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L29812) to handle warning notifications. Can be
...
about the just-pushed file can be retrieved by examining [`tpp_lexer_getfile(LEXER)`](../src/tpp-amalgamation.h#L27186).

Notes:

- This hook can be used by a frontend to implement stuff like GCC's `--trace-includes`.
- This hook is *NOT* called for [`tpp_file_subtext_push()`](../src/tpp-amalgamation.h#L21832) or [`tpp_file_pushdummy()`](../src/tpp-amalgamation.h#L22256)
...
by examining [`tpp_lexer_getfile(LEXER)`](../src/tpp-amalgamation.h#L27186).

Notes:

- When files are popped by [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L28798) within a
  [`tpp_lexer_manualpopfile_start()`](../src/tpp-amalgamation.h#L28796)-region, this hook is called during the
  *commit* phase (i.e.: by [`tpp_lexer_manualpopfile_break_commit()`](../src/tpp-amalgamation.h#L28803)),
  rather than [`tpp_lexer_manualpopfile_popfile()`](../src/tpp-amalgamation.h#L28798) as one might suspect at first.
- This hook is *NOT* called by [`tpp_file_subtext_pop()`](../src/tpp-amalgamation.h#L21858) or [`tpp_file_popdummy()`](../src/tpp-amalgamation.h#L22270)
...
been populated by [`tpp_lexer_yieldraw_at_include_string_blocking()`](../src/tpp-amalgamation.h#L28974) (and macros
were also already expanded), and the current token is [`TPP_TOK_INCPATH_DQUOTE`](../src/tpp-amalgamation.h#L28943) or
[`TPP_TOK_INCPATH_LANGLE`](../src/tpp-amalgamation.h#L28944).
...
To gain access to the `#include`-string, you must use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L29009)
...
the error (by returning something other than [`TPP_ENOENT`](../src/tpp-amalgamation.h#L13692)), or log the error to
implement something like GCC's `-MG` commandline switch.

This hook is called just before [`TPP_W_NO_SUCH_FILE`](../src/tpp-amalgamation.h#L4900) would be emitted, with the lexer's
current token still being the `<stdio.h>` or `"file.h"` string, meaning if you want
to know what that string says, you can use [`tpp_lexer_decode_include_string_cb()`](../src/tpp-amalgamation.h#L29009) to decode it.
...
- This hook is *ONLY* invoked when `#define` is encountered, or
  ...
  definition.
- Calls to [`tpp_lexer_define()`](../src/tpp-amalgamation.h#L28442) or other related functions will
...
no predefined definition: [`_TPP_KEYWORD_MACRO_UNDEFINED`](../src/tpp-amalgamation.h#L22617)). This
hook is imply called as part of the process of evaluating `#undef`

This hook is *ONLY* invoked when `#undef` is encountered.
Calls to [`tpp_lexer_undef()`](../src/tpp-amalgamation.h#L28454), [`tpp_keyword_undef()`](../src/tpp-amalgamation.h#L23185), or other
...
Extra callback invoked by [`tpp_lexer_foreach_include_path()`](../src/tpp-amalgamation.h#L29040) at different
...
Extra callback invoked by [`tpp_lexer_foreach_embed_path()`](../src/tpp-amalgamation.h#L29082) at different points
...
Called by [`tpp_lexer_decodestring()`](../src/tpp-amalgamation.h#L29571) when an unknown `\`-escape sequence is encountered.
...
Called by [`tpp_lexer_warnf()`](../src/tpp-amalgamation.h#L29812) just before it's about to return [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L13740)
This hook can be used to do additional state changes that may be necessary by the
hosting application in order to handle the resulting [`TPP_ELEXERROR`](../src/tpp-amalgamation.h#L13740)
...
Called by [`tpp_lexer_yieldraw()`](../src/tpp-amalgamation.h#L28638) when [`TPP_HAVE_SMART_FLOAT_TOKENS`](config-conf.md#tpp_have_smart_float_tokens) is enabled and
...
yet, though can be loaded using [`tpp_lexer_readchar()`](../src/tpp-amalgamation.h#L28543) and [`tpp_lexer_readunichar()`](../src/tpp-amalgamation.h#L28556))
