...
| ------- | ------ | ------------- | ------------------------ | ----------- |
| `??/` | pre_stdc-C23 | `TRIGRAPHS` | `-ftrigraphs` | Textual replacements for various `??x` sequences; e.g. `??/` behaves the same as `\` |
| `<%` | C95+ | `DIGRAPHS` | `-fdigraphs` | Token replacements for some 2-char sequences like `<%` being replaced with `{` |
...
| `,##__VA_ARGS__` | GCC | `VA_GLUE_COMMA_IN_MACROS` | `-fglue-comma-in-macros` | GCC-style va-comma: `#define printf(format, ...) fprintf(stderr, format, ##__VA_ARGS__)` |
| `?:` | GCC | `BUILTIN_EXPR_IF_ELSE_OPTIONAL_TT` | `-fif-else-optional-true` | GCC-style if-else in expressions: `#if foo ?: bar` |
| `__VA_OPT__` | C++20 | `VA_OPT_IN_MACROS` | `-fva-opt-in-macros` | C++20 `__VA_OPT__`: `#define printf(format, ...) fprintf(stderr, format __VA_OPT__(,) __VA_ARGS__)` |
...
| `#embed` | C23 / C++26 | `CPP_EMBED` | `-fembed-directives` | C++26 `#embed "resource.dat" limit(10)` directive |
...
| `__VA_COMMA__` | TPP | `VA_COMMA_IN_MACROS` | `-fva-comma-in-macros` | Expands to `,` if varargs are non-empty: `#define printf(format, ...) fprintf(stderr, format __VA_COMMA__ __VA_ARGS__)` |
