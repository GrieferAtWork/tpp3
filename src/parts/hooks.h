...
for (local doc, name,
	...
	if (!builtin_FOO_HOOK) {
		...
		if ("cookie" in prototypeArgs) {
			print("#if TPP_HAVE_HOOK_COOKIES");
			print("	tpp_hook_cookie TPP_INTERNAL(thle_cookie); /" "* [?..?] Cookie argument for `thle_cb` *" "/");
			print("#endif /" "* TPP_HAVE_HOOK_COOKIES *" "/");
		}
...
for (local doc, name,
	...
	if (!builtin_FOO_HOOK) {
		...
		local extraParams = fixProto(prototypeSuffix.lstrip(")").lstrip().lstrip("(").rstrip(")").strip()
			.rereplace(r"\btpp_hook_cookie\s+cookie\s*,\s*", "")
			.rereplace(r"\s*,\s*tpp_hook_cookie\s+cookie\b", "")
			.rereplace(r"\btpp_hook_cookie\s+cookie\b", ""));
#if 1
		print("#define tpp_hooks_call_", name.lower(), "(", formattedParams, ") \\");
		print("	_tpp_hooks_call_", name.lower(), "(lexer",
