...
for (local featureName, TPP_HAVE_FOO: features) {
	print("#if ", TPP_HAVE_FOO, " < 0");
	print("		unsigned int TPP_INTERNAL(tff_", featureName, "): 1;");
	print("#define _tpp_features_get_", featureName, "(self) tpp_expect((self)->TPP_INTERNAL(tf_flags).TPP_INTERNAL(tff_", featureName, "), ", TPP_HAVE_FOO, " == -1)");
